...
**Variable `DEPLOY_NPM_BIN` (recommended):** set a repository **Actions variable** to the **full path of `npm`** on the server for the **same Node** as your systemd `ExecStart` (e.g. `/home/dictionary/.nvm/versions/node/v22.18.0/bin/npm`). On the server, after `nvm use` / the Node you use in the unit, run `command -v npm` to copy the path. This makes `npm ci` during deploy build **better-sqlite3** for the correct ABI; without it, SSH may use a different default Node and break the app after each deploy.
