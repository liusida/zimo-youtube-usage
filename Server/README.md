...
**Native modules (`better-sqlite3`):** `npm ci` in CI runs via `bash -lc` so it uses the same **Node** as an interactive login (e.g. nvm). That matches `systemd` if `ExecStart` uses that same Node. If you ever see `NODE_MODULE_VERSION` / `ERR_DLOPEN_FAILED`, rebuild on the server with the Node that runs the service, e.g. `bash -lc 'cd ~/zimo-usage && rm -rf node_modules && npm ci --omit=dev'`, then restart the unit.
