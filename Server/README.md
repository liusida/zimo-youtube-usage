...
**`DEPLOY_NPM_BIN` (recommended):** set a repository **secret** or **variable** (same name) to the **full path of `npm`** on the server for the **same Node** as systemd `ExecStart` (e.g. `/home/dictionary/.nvm/versions/node/v22.18.0/bin/npm`). The workflow prefers the **secret** if both are set. On the server, run `command -v npm` with that Node active to copy the path. This makes deploy-time `npm ci` use the correct ABI for **better-sqlite3**; without it, SSH often uses `/usr/bin/npm` (e.g. Node 18) and breaks the app.
